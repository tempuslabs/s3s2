    ...
    stages {
        ...
        stage('publish') {
            ...
            steps {
                ...
                sh script: 'curl --fail --user "${NEXUS_CREDS}" --upload-file ./gopath/linux/s3s2-linux-amd64 ${NEXUS_PATH}/s3s2-linux-amd64', label: "Publishing Linux build"
                sh script: 'curl --fail --user "${NEXUS_CREDS}" --upload-file ./gopath/darwin/s3s2-darwin-amd64 ${NEXUS_PATH}/s3s2-darwin-amd64', label: "Publishing Mac build"
                sh script: 'curl --fail --user "${NEXUS_CREDS}" --upload-file ./gopath/windows/s3s2-windows-amd64.exe ${NEXUS_PATH}/s3s2-windows-amd64.exe', label: "Publishing Windows build"
            }
        }
    }
}
